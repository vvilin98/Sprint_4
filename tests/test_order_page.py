...
class TestYaScooterOrderPage:
    ...
    @allure.description('Проверка что при корреткных заполненных данных на этапе "Про аренду", '
                        'нажатии на кнопку "Заказать", происходит оформление заказа')
    ...
    @allure.title('Заполнение данных на этапе "Про аренду",оформление заказа переход на страницу с заказом')
